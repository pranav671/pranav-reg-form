...
export default function Details(props) {
  ...
  const generateLink = () => {
    axios
      .get(server_url + "/newLink/" + data.id)
      .then((res) => {
  ...
  return code == "" || (data != null && data.id == "Error") ? (
    <>
    ...
    <div style={{ color: "black", marginInline: "auto" }}>
      ...
      >
        ...
        <h5 className="mt-3 ms-5">
          Payment status = {data.hasPaid ? "Successful" : "Pending"}
        </h5>
        <p className="mt-3 ms-5">
          Payment Link =
          {data.paymentUrl == null || data.paymentUrl == "error" 
            ?(<button className="btn btn-outline-success ms-2" onClick={generateLink}>Generate</button>)
...
const Card = (props) => {
  ...
  return (
    <div className="card-body ps-5">
      ...
      <p>
        School = {data.school}
        <a className="user-select-none" style={{ color: "#fbfbfb" }}>
          - - - - -{" "}
        </a>
        Std - {data.standard}
      </p>
